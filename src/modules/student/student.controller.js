...
export const getHalakaDetails = async (req, res) => {
  try {
    ...
    const finishedSessions = attendance.filter((a) =>
      ["present", "late", "excused"].includes(a.status)
    ).length;

    const upcomingSessions = halaka.getUpcomingSessions
      ? halaka.getUpcomingSessions(5)
      : [];
...
export const getTodayHalakatForStudent = async (req, res) => {
  try {
    ...
    // Query filter for halakat scheduled today, enrolled student
    const filter = {
      $and: [
        {
          $or: [
